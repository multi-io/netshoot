...
RUN apt update && \
    ...
    DEBIAN_FRONTEND=noninteractive apt -yy install sudo git less vim curl wget net-tools iputils-ping iputils-arping iputils-tracepath dnsutils tcpdump whois netcat docker.io unrar cu psmisc iptraf nethogs libpcap-dev nmap strace lsof rsync jq etcd-client s3cmd
